...
def optimize_camera_settings(cam_id=0):
    """
    Optimize camera settings for pellet detection using OpenCV.
    
    IMPORTANT: Uses OpenCV which has different scale than v4l2-ctl:
    - OpenCV uses 0-255 scale for most controls
    - OpenCV automatically translates to camera's hardware scale
    ...
    print(f"⚙️ Optimizing camera {cam_id} settings via OpenCV...")
    
    # DO NOT disable autofocus - leave it as is or enable it
    # cap.set(cv2.CAP_PROP_AUTOFOCUS, 1) would enable it
    
    # Set brightness to middle (OpenCV 0-255 scale)
    cap.set(cv2.CAP_PROP_BRIGHTNESS, 128)
    
    # Set contrast (OpenCV 0-255 scale)
    cap.set(cv2.CAP_PROP_CONTRAST, 128)
    
    # Set saturation (OpenCV 0-255 scale)
    cap.set(cv2.CAP_PROP_SATURATION, 128)
    
    # Set sharpness (OpenCV 0-255 scale)  
    cap.set(cv2.CAP_PROP_SHARPNESS, 128)
    
    # Maximize frame rate
    cap.set(cv2.CAP_PROP_FPS, 30)
    
    # Disable automatic gain for best quality
    cap.set(cv2.CAP_PROP_GAIN, 0)
    
    print(f"✅ Camera {cam_id} optimized via OpenCV")
    print(f"   Note: OpenCV auto-translates 0-255 values to camera's hardware scale")
